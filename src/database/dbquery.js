export const insertnewUser = `insert into users (email ,user_name,is_login) values ($1,$2,$3) on conflict (email) do nothing returning user_id`
export const getUserid = `select user_id from users where email=$1`
export const updateLogin = `update users set is_login=$2 where email=$1 returning user_id`
export const logout = `update users set is_login=$1 where user_id = $2`
...
export const getCapsulesInDescOrder = 'select * from capsules where user_id=$1 and created_at < $2 and is_deleted=false order by created_at desc limit 12'
export const getCapsulesInAscOrder = 'select * from capsules where user_id=$1 and created_at > $2 and is_deleted=false order by created_at asc limit 12'
export const getCapsulessortbySizeInAsc = 'select * from capsules where user_id=$1 and capsule_size > $2 and is_deleted=false order by capsule_size asc limit 12'
export const getCapsulessortbySizeInDesc = 'select * from capsules where user_id=$1 and capsule_size < $2 and is_deleted=false order by capsule_size desc limit 12'
...
export const insertfragmentfile = 'insert into fragments (capsule_id,size,tag,url,fragment_type,file_name) values ($1, $2, $3, $4,$5,$6) returning fragment_id , size , url';
